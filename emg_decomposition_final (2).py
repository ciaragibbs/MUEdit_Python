...
class offline_EMG(EMG):
    ...
    def electrode_formatter(self):
        ...
        for i in range(self.signal_dict['nelectrodes']):
            ...
            # bandpass filtering
            self.signal_dict['filtered_data'][chans_per_electrode[i]*(electrode-1):electrode*chans_per_electrode[i],:] = bandpass_filter(self.signal_dict['filtered_data'][chans_per_electrode[i]*(electrode-1):electrode*chans_per_electrode[i],:],self.signal_dict['fsamp'],emg_type = self.emgopt)        
...
################################ CONVOLUTIVE SPHERING ########################################
    def convul_sphering(self,g,interval,tracker):
        ...
        if self.to_filter: # adding since will need to avoid this step if doing real-time decomposition + biofeedback, but fine for offline analysis
            ...
            self.signal_dict['batched_data'][tracker] = bandpass_filter(self.signal_dict['batched_data'][tracker],self.signal_dict['fsamp'],emg_type = self.emgopt)  
